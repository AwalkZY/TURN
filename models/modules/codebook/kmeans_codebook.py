...
class KmeansVectorQuantizer(nn.Module):
    def __init__(
        self, dim, vq_dim, num_vars, groups, combine_groups=False, time_first=True, gamma=0.25, **kwargs
    ):
        ...
        self.gamma = gamma
        self.mse_mean = nn.MSELoss(reduction="none")
    ...
    def forward_idx(self, x):
        ...
        loss = res["kmeans_loss"]
        return res["x"], res["prob"], loss
    ...
    def forward(self, x, produce_probs=False):
        ...
        if produce_probs:
            result["prob"] = (-d.permute(1, 2, 3, 0)).softmax(-1)
        # (B, T, G, S)
