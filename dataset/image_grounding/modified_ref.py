...
from utils.calculator import box2map, sigmoid_norm, softmax_norm
    ...
    if split == 'train':
        ...
        return T.Compose([
            T.RandomSelect(
                ...
                T.Compose([
                    T.RandomResize([320, 448, 576], with_long_side=False),
                    T.RandomSizeCrop(288, 576),
                    T.RandomResize(scales),
                ]),
                p=crop_prob
            ),
...
class ReferDataset(data.Dataset):
    ...
    def __init__(self, data_root, vocab_path, use_weight=True, augment=None, split_root='data', dataset='referit',
                 imsize=416, return_idx=False, testmode=False, split='train', max_query_len=40, **kwargs):
        self.images = []
        ...
        weight_path = osp.join(osp.join(self.split_root, self.dataset), "weight.pt")
        if self.use_weight and not osp.exists(weight_path):
            raise FileNotFoundError
        if self.use_weight:
            self.weight = softmax_norm(torch.tensor(torch.load(weight_path)), reverse=False)
            # WARNING: score越高，越倾向于是audio相关的item
        else:
            self.weight = torch.ones(len(self.images))
    ...
    def __getitem__(self, idx):
        ...
        if self.testmode:
            ...
            return img, np.array(img_mask), np.array(word_id, dtype=int), \
                   np.array(word_mask, dtype=int), np.array(bbox, dtype=np.float32), idx

        # heatmap = box2map([bbox.numpy()], max_size=self.imsize)

        # if self.testmode:
        #     return img, np.array(word_id, dtype=int), np.array(word_mask, dtype=int), \
        #            heatmap, self.images[idx][0]
        # else:
        #     # print(img.shape)
        #     return img, np.array(img_mask), np.array(word_id, dtype=int), \
        #            np.array(word_mask, dtype=int), heatmap
