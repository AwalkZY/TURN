...
def box2map(boxes, max_size, name=None):
    if name == "SoundNet":
        ...
        for bbox in boxes:
            ...
            cur_map[y_min:y_max, x_min:x_max] = 1
            gt_map += cur_map
        gt_map /= 2
        gt_map[gt_map > 1] = 1
        return gt_map
    elif name is None or name == "VGGSS" or name == "MUSIC":
        gt_map = np.zeros([max_size, max_size])
        for bbox in boxes:
            cur_map = np.zeros([max_size, max_size])
...
def sigmoid_norm(inputs, reverse, alpha=1):
    result = (-inputs).sigmoid() if reverse else inputs.sigmoid()
    return (alpha + (result / result.max(-1, keepdim=True)[0])) / (1 + alpha)


def softmax_norm(inputs, reverse, k=5):
    tau = (inputs.max() - inputs.min()) / math.log(k)
    results = (-inputs / tau).softmax(-1) if reverse else (inputs / tau).softmax(-1)
    return results / results.max(-1, keepdim=True)[0]
...
def max_min_norm(x: torch.Tensor, dim):
    return (x - x.min(dim=dim, keepdim=True)[0]) / (
            x.max(dim=dim, keepdim=True)[0] - x.min(dim=dim, keepdim=True)[0] + 1e-9)
